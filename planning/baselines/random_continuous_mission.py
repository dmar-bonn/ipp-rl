...
class RandomContinuousMission(Mission):
    def __init__(
        self,
        mapping: Mapping,
        ...
        self.mission_name = "Random Continuous"
        self.mission_type = MissionType.RANDOM_CONTINUOUS

    def create_waypoints(self) -> Tuple[np.array, List]:
        """
        Generates waypoints sampled uniformly at random in by 3D volume bounded by dist_to_boundaries in planar xy-
        direction and bounded by min_altitude and max_altitude in z-direction until mission budget is exceeded.

        Returns:
            (np.array): sampled waypoints uniformly at random
        """
        min_y_pos = self.dist_to_boundaries
        max_y_pos = self.mapping.grid_map.x_dim * self.mapping.grid_map.resolution - self.dist_to_boundaries

        min_x_pos = self.dist_to_boundaries
        max_x_pos = self.mapping.grid_map.y_dim * self.mapping.grid_map.resolution - self.dist_to_boundaries
        ...
        while remaining_budget >= 0:
            ...
            trials = 100
            while (
                remaining_budget - action_costs(waypoint, previous_waypoint, self.uav_specifications) < 0 and trials > 0
            ):
                trials -= 1
                waypoint = np.random.uniform(
                    low=(min_y_pos, min_x_pos, self.min_altitude),
